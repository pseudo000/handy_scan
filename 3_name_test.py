...
class SpreadsheetGUI(QMainWindow):
    def __init__(self):
        ...
        self.tree.setHeaderLabels(['NO.', 'HOUSE NO.', '荷主', '品名','重量', '区分', 'CT', 'SCANNED', 'COMMENT'])
        ...
        self.tree.setColumnWidth(8, 120)
            ...
            for i, row in enumerate(
                ...
                if row[2]:  # E열에 데이터가 있는 경우에만 해당 컬럼에 불러오기
                    item = QTreeWidgetItem(
                        [str(i), str(row[1]), str(row[5]), str(row[6]), str(row[19]), str(row[21]),str(row[13])]
                    )
                    item.setText(1, str(row[5]))  # HOUSE NO.
                    item.setText(2, str(row[6]))  # 荷主
                    item.setText(3, str(row[13]))  # 品名
                    item.setText(4, str(row[21]))  # 重量
                    item.setText(5, str(row[1]))  # 区分
                    item.setText(6, str(row[19]))  # CT
        ...
        if selected_files:
            ...
            for i, row in enumerate(
                ...
                if row[4]:  # E열에 데이터가 있는 경우에만 해당 컬럼에 불러오기
                    item = QTreeWidgetItem(
                        [str(i), str(row[1]), str(row[6]), str(row[9]), str(row[17]), str(row[18]),str(row[13])]
                    )
                    item.setText(1, str(row[6]))  # HOUSE NO.
                    item.setText(2, str(row[9]))  # 荷主
                    item.setText(3, str(row[13])) #品名
                    item.setText(4, str(row[18]))  # 重量
                    item.setText(5, str(row[1]))  # 区分
                    item.setText(6, str(row[17]))  # CT
                    self.tree.addTopLevelItem(item)
            ...
            for i in range(self.tree.topLevelItemCount()):
                ...
                for row in csv_data:
                    ...
                    if house_no == modified_row:
                        item.setText(7, row[5])

                        sum_value = csv_sum_dict[modified_row]
                        item.setText(7, str(sum_value))

                    if not item.text(7) or int(item.text(6)) > int(item.text(7)):
                        item.setText(8, "이상유무 확인 바랍니다.")
                    else:
                        item.setText(8, "OK")

                    item.setForeground(8, QtGui.QColor('red'))
                    font = QtGui.QFont()
                    font.setBold(True)
                    item.setFont(8, font)
    ...
    def update_column_sums(self):
        ...
        for i in range(total_rows):
            item = self.tree.topLevelItem(i)
            column_sum_4 += float(item.text(4))
            column_sum_6 += float(item.text(6))
            if item.text(7):  # 7번째 컬럼 데이터 확인
                column_sum_7 += float(item.text(7))
                scan_count += 1

        # set column sum values to the status bar
        column_sum_text = f'<html><body><span style="font-weight: bold; color: red; font-size: x-large;">■총 건수: {total_rows}  ■총 CT수: {int(column_sum_6)}  ■총 중량: {column_sum_4:.2f}</span></body></html>'
        additional_text = f'<br/><span style="font-weight: bold; color: blue; font-size: x-large;">■스캔 건수: {scan_count}  ■스캔 CT수: {int(column_sum_7)}</span>'
    ...
    def save_data(self):
        ...
        new_file_path, _ = QFileDialog.getSaveFileName(self, 'Save File As', "Result", 'Excel files (*.xlsx)')
